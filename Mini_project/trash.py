...
if page == "Overview":
    ...
    TOTAL_CRASHES = execute_query("""
    SELECT COUNT(*) AS TOTAL_CRASHES
    FROM traffic_crash_data
    """).iloc[0,0]

    TOTAL_INJURIES = execute_query("""
    SELECT SUM(INJURIES_TOTAL) AS TOTAL_INJURIES
    FROM traffic_crash_data
    """).iloc[0,0]

    TOTAL_FATALITIES = execute_query("""
    SELECT SUM(INJURIES_FATAL) AS TOTAL_FATALITIES
    FROM traffic_crash_data
    """).iloc[0,0]

    SERIOUS_INJURIES = execute_query("""
    SELECT SUM(INJURIES_INCAPACITATING) AS SERIOUS_INJURIES
    FROM traffic_crash_data
    """).iloc[0,0]

    MOST_COMMON_CRASH_TYPE = execute_query("""
    SELECT CRASH_TYPE
    FROM traffic_crash_data
    GROUP BY CRASH_TYPE
    ORDER BY COUNT(*) DESC
    ...
    TOP_CRASH_BEAT = execute_query("""                        
    SELECT BEAT_OF_OCCURRENCE,COUNT(*) AS TOTAL_CRASHES
    FROM traffic_crash_data
    ...
    TOP_TRAFFIC_WAY_TYPE = execute_query("""                        
    SELECT TRAFFICWAY_TYPE,COUNT(*) AS TOTAL_CRASHES
    FROM traffic_crash_data
    ...
    col2.metric("TOTAL INJURIES", f"{int(TOTAL_INJURIES):,}")
    col3.metric("TOTAL FATALITIES ",f"{int(TOTAL_FATALITIES):,}")
    col4.metric("SERIOUS INJURIES ",f"{int(SERIOUS_INJURIES):,}")


    st.info(f"Most Common Crash Type: {MOST_COMMON_CRASH_TYPE}")
    st.info(f"Most CRASH BEAT: {TOP_CRASH_BEAT}")
    st.info(f"TOP TRAFFIC WAY TYPE: {TOP_TRAFFIC_WAY_TYPE}")
    ...
    if st.button("Run Query"):
        with st.spinner("Fetching Data..."):
            ...
            if selected_query =="Top 5 most dangerous combinations of weather and crash type":
                ...
                st.info("These zones represent major crash hotspots due to frequent crashes. ")
